    ...
    public class OrderController : Controller
    {
        ...
        public IActionResult GetAll(string status)
        {
            ...
            if (response != null && response.IsSuccess)
            {
                ...
                {
                    case "approved":
                        orderList = orderList.Where(u => u.Status == SD.Status_Approved).ToList();
                        break;
                    case "readyforpickup":
                        orderList = orderList.Where(u => u.Status == SD.Status_ReadyForPickup).ToList();
                        break;
                    case "cancelled":
                        orderList = orderList.Where(u => u.Status == SD.Status_Cancelled || u.Status == SD.Status_Refunded).ToList();
                        break;
                    default:
                        break;
                }
